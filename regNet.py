...
class RegNet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.feature_matching = nn.Sequential(
            nin_block(576, 512, kernel_size=5, strides=1, padding=2),
            nn.MaxPool2d(3, stride=2),
    ...
    def forward(self, img_data, lidar_data):
        # IMG branch
        img_data = F.max_pool2d(self.rgb_features(img_data), 3, 2)
        # Lidar branch
        lidar_data = F.max_pool2d(self.depth_features(lidar_data), 3, 2)

        # Data fusion
        fused_data = torch.cat((lidar_data, img_data), 1)
        fused_data = F.max_pool2d(self.feature_matching(fused_data), 3, 2)
